...
const UploadModal = () => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (values) => {
    try {
      ...
      const { data: songData, error: songError } = await supbaseClient.storage
        .from('songs')
      ...
      const { data: imageData, error: imageError } = await supbaseClient.storage
        .from('images')
      ...
      const { error: supabaseError } = await supbaseClient.from('songs').insert({
        user_id: user.id,
        title: values.title,
